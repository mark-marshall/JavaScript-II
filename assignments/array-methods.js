...
let ticketPriceTotal = runners.reduce(function(totalAcc, runner) {
  return totalAcc + runner.donation;
}, 0);
...
// Problem 1
// Calculate the average donation per runner and then add a new key:value pair to see whether runners are above or below average donation

let averageCalculation =
  runners.reduce(function(totalSoFar, runner) {
    return totalSoFar + runner.donation;
  }, 0) / runners.length;
console.log(averageCalculation);

let updatedRunnersList = runners.map(function(runner) {
  return {
    id: runner.id,
    first_name: runner.first_name,
    last_name: runner.last_name,
    email: runner.email,
    shirt_siz: runner.shirt_size,
    company_name: runner.company_name,
    donation: runner.donation,
    donation_above_average: runner.donation > averageCalculation
  };
});

console.log(updatedRunnersList);

// Problem 2
// Create an array of email addresses so that the organisers can readily email all of the participants

let emailBook = runners.map(function(runner) {
  return runner.email;
});
console.log(emailBook);

// Problem 3
// Sort the runners list into buckets for bib collection by alphabetical surname

let acList = runners.filter(function(runner) {
  return (
    runner.last_name.charAt(0) === "A" ||
    runner.last_name.charAt(0) === "B" ||
    runner.last_name.charAt(0) === "C"
  );
});

let dfList = runners.filter(function(runner) {
  return (
    runner.last_name.charAt(0) === "D" ||
    runner.last_name.charAt(0) === "E" ||
    runner.last_name.charAt(0) === "F"
  );
});

let giList = runners.filter(function(runner) {
  return (
    runner.last_name.charAt(0) === "G" ||
    runner.last_name.charAt(0) === "H" ||
    runner.last_name.charAt(0) === "I"
  );
});
